	      ...
	      xd3_posix_func *func, size_t *nread)
{
  ...
  while (nproc < size)
    {
      size_t tryread = XDELTA3_MIN(size - nproc, 1U << 30);
...
main_merge_output (xd3_stream *stream, main_file *ofile)
{
  ...
  while (inst_pos < stream->whole_target.instlen || !at_least_once)
	     ...
	     inst_pos < stream->whole_target.instlen)
	{
	  ...
	  usize_t take = XDELTA3_MIN(inst->size, window_size - window_pos);
	  xoff_t addr;
	  ...
	  switch (inst->type)
		...
		{
		  if (window_srcset) {
		    window_srcmin = XDELTA3_MIN(window_srcmin, inst->addr);
		    window_srcmax = XDELTA3_MAX(window_srcmax, inst->addr + take);
		  } else {
		    window_srcset = 1;
				 ...
				 size_t     *nread)
{
  int ret;
  usize_t i;
  usize_t try_read = XDELTA3_MIN (input_size, XD3_ALLOCSIZE);
...
main_get_winsize (main_file *ifile) {
  ...
  if (main_file_stat (ifile, &file_size) == 0)
    {
      size = (usize_t) XDELTA3_MIN(file_size, (xoff_t) size);
    }

  size = XDELTA3_MAX(size, XD3_ALLOCSIZE);
  ...
  do
    {
      ...
      try_read = (usize_t) XDELTA3_MIN ((xoff_t) config.winsize, input_remain);
